...
-- Builds the DE214 database, its RAW / DEV / PROD schemas, and loads
...
CREATE DATABASE IF NOT EXISTS DE214;

CREATE SCHEMA IF NOT EXISTS DE214.RAW;
CREATE SCHEMA IF NOT EXISTS DE214.DEV;
CREATE SCHEMA IF NOT EXISTS DE214.PROD;
DROP SCHEMA IF EXISTS DE214.PUBLIC;

-- ---------------------------------------------------------------------------
-- Customer master  (<- TPC-H CUSTOMER)
-- ---------------------------------------------------------------------------
CREATE OR REPLACE TABLE DE214.RAW.C_MST AS
SELECT
    c_custkey     AS c_k,
    c_name        AS c_nm,
...
-- ---------------------------------------------------------------------------
CREATE OR REPLACE TABLE DE214.RAW.ORD_HDR AS
SELECT
    o_orderkey      AS o_k,
    o_custkey       AS o_cust,
    o_orderdate     AS o_dt,
...
-- ---------------------------------------------------------------------------
CREATE OR REPLACE TABLE DE214.RAW.LN_ITM AS
SELECT
    l_orderkey      AS l_ord,
...
-- ---------------------------------------------------------------------------
CREATE OR REPLACE TABLE DE214.RAW.GEO AS
SELECT
...
-- Quick sanity check
-- ---------------------------------------------------------------------------
SELECT 'C_MST'   AS tbl, COUNT(*) AS row_cnt FROM DE214.RAW.C_MST
UNION ALL SELECT 'ORD_HDR', COUNT(*) FROM DE214.RAW.ORD_HDR
UNION ALL SELECT 'LN_ITM',  COUNT(*) FROM DE214.RAW.LN_ITM
UNION ALL SELECT 'GEO',     COUNT(*) FROM DE214.RAW.GEO
ORDER BY tbl;

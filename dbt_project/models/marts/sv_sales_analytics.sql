...
METRICS (
    ...
    orders.discount_rate AS 1 - (SUM(orders.net_revenue) / NULLIF(SUM(orders.gross_revenue), 0))
        WITH SYNONYMS = ('discount percentage', 'average discount', 'discount ratio')
        COMMENT = 'Blended discount rate: 1 - (net_revenue / gross_revenue). Returns NULL when gross_revenue is zero.'
)
